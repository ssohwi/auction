...
module.exports = class Good extends Sequelize.Model { // Model : static init과 static associate으로 나뉨
  static init(sequelize) { // static init : 테이블에 대한 설정
    return super.init({ // super.init의 첫번째 인수 : 테이블 컬럼 설정
      ...
      modelName: 'Good', // 노드 모델 이름
      tableName: 'goods', // 실제 db 테이블 이름
  ...
  static associate(db) { // static associate : 다른 테이블과의 관계
    ...
    db.Good.belongsTo(db.User, { as: 'Owner' }); // 등록한 유저, OwnerId컬럼 생성
    db.Good.belongsTo(db.User, { as: 'Sold' }); // 낙찰받은 유저, SoldId컬럼으로 생성
